...
The payoff of the Price decomposition comes from setting $z = R$ (Bayes risk). Because fitness is a decreasing linear function of risk (\cref{eq:expected-fitness}), the selection covariance becomes a negative-definite term. Thus, the selection component always pushes mean risk down, while mutation/transmission can offset part of that decrease.
...
\end{equation}
\emph{Selection only:} $\Delta \Rbar = -T \cdot \Var_\pi(R)/\Wbar \le 0$. Mean Bayes risk strictly decreases unless all types in support have equal Bayes risk.
\end{theorem}
...
\]
where $\tilde{\varepsilon} = N\varepsilon(1 - 1/M)$ is the effective per-encoding mutation rate (the product of per-position rate $\varepsilon$, number of positions $N$, and probability of changing to a different value $1 - 1/M$),
...
x^*_{\mathcal{K}^*}(p) = \frac{\tilde{\varepsilon} \cdot W(p^*_\mathcal{K})\, Q_{1,\mathcal{K}^*}(p^*_\mathcal{K} \to p)}{W(p^*_\mathcal{K}) - W(p)} + O(\varepsilon^2).
\end{equation}
