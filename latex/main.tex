...
The qualitative motivation for our main result is consistent with statistical learning theory: shared representations can improve cross-task generalisation. \citet{baxter_2000} analysed inductive bias learning in environments of related tasks and derived sample-complexity improvements from multi-task training. \citet{maurer_etal_2016} derived finite-sample excess-risk bounds characterising regimes where multi-task representation learning outperforms independent task learning. In the information-theoretic literature, related ideas appear as optimal data reduction via sufficient statistics.
...
For each task, write its evaluation vector on $W$ as $v_t := (f_t(w_1), \ldots, f_t(w_N)) \in \R^N$, so $v_t^\top$ is the $t$-th row of $F$.
...
\end{itemize}
\item Let $\nu$ be the sampling law of $v_t$ (the pushforward of the task law under evaluation on $W$). If $\nu \ll \lambda^N$ (absolutely continuous w.r.t.\ Lebesgue measure on $\R^N$), then with probability $1$ a single task has distinct values on all $N$ states, so $k_1 = N$ and the cascade is trivial. Thus biologically meaningful phase transitions require structured task families.
\end{enumerate}
\end{theorem}
...
(d) By assumption, $v_1 \sim \nu$ with $\nu \ll \lambda^N$. For each $i \ne j$, the tie event $\{v_{1i} = v_{1j}\}$ is the hyperplane $H_{ij} = \{x \in \R^N : x_i = x_j\}$, which has Lebesgue measure $0$. Hence $\nu(H_{ij}) = 0$. A finite union bound over the $\binom{N}{2}$ pairs gives $P(\exists i \ne j: v_{1i} = v_{1j}) = 0$, so the coordinates are pairwise distinct almost surely.
\end{proof}

\begin{corollary}[Capacity-Aware Transition]\label{cor:capacity}
Define empirical ecological complexity at task count $T$ by
\[
k_T := |W/{\sim_T}|.
\]
Then the sequence $(k_T)_{T \ge 1}$ satisfies:
\begin{itemize}
...
\begin{proof}
The random variables $R(p, f_t)$ are iid with $\E[R(p, f_t)] = R(p) > 0$ and $0 \le R(p, f_t) \le B^2$ (since $\Var(f \mid C_x) \le \E[f^2 \mid C_x] \le B^2$). By Hoeffding's inequality for bounded iid variables in $[0, B^2]$ \citep{hoeffding_1963}:
\[
...
Let $z\colon \Omega \to \R$ be any real-valued trait of encodings. The change in population mean of $z$ across one generation satisfies the Price identity \citep{price_1970,price_1972}:
...
\begin{proof}
This is the standard Price equation derivation \citep{price_1970,price_1972}. Write $\pi'(p)$ for the frequency after selection but before mutation:
\[
...
\Cref{fig:risk-trajectories} confirms the Price decomposition computationally. Recall that Bayes risk quantifies how poorly an encoding serves the task ecology: $R(p) = 0$ means the encoding preserves all task-relevant distinctions (full ecological veridicality), while $R(p) > 0$ means ecologically meaningful information is lost. \Cref{fig:risk-trajectories} shows that mean Bayes risk declines over generations for all task-diversity levels $T$, consistent with a dominant negative selection term in \eqref{eq:risk-decrease} and with the mean-field tendency of \cref{thm:price-risk}. All four trajectories converge to approximately the same asymptotic risk (${\approx}\,0.08$), which is the capacity floor imposed by $M = 2 \ll N = 11$: with only two percepts, the best achievable encoding is the optimal $2$-cell partition (\cref{thm:lossy}), regardless of how many tasks are sampled. What $T$ controls is not the asymptotic level under this update rule but the \emph{noise}: higher $T$ gives a more precise per-generation estimate of the population risk, reducing the stochastic variance visible in the trajectories ($T=1$, blue, fluctuates widely; $T=1000$, red, is nearly deterministic).
...
\emph{Step 1 (Frobenius decomposition).} Permute $A$ into lower block-triangular form, separating a transient block $T$ from closed communicating-class blocks $B_1, \ldots, B_L$ \citep{seneta_2006}. The closed-part dynamics decomposes into independent primitive blocks plus transient feeding terms that decay exponentially (\cref{lem:frobenius}).

\emph{Step 2 (Perron expansion on each block).} Each primitive block $B_i$ has a simple dominant eigenvalue $\lambda_i$ with positive left and right eigenvectors. The Perron--Frobenius theorem gives $B_i^g = \lambda_i^g v_i u_i^\top + O(r_i^g)$ with $r_i < \lambda_i$ \citep{seneta_2006} (\cref{lem:perron-expansion}).

\emph{Step 3 (Dominant class wins).} Among all reachable closed classes, the one with the largest $\lambda_i$ eventually dominates the unnormalised iterate $A^g x_0$: all other contributions decay exponentially relative to $\lambda_*^g$ \citep{seneta_2006}. After normalisation, the population concentrates on the Perron profile of $\mathcal{K}^*$, proving (a).
...
\Cref{thm:quasispecies} is a statement about the deterministic mean-field limit. Real populations are finite and stochastic \citep{park_munoz_deem_2010}. The next result closes the gap: the Wright--Fisher process tracks the deterministic limit on any fixed time horizon as the population size $K$ grows, with $O(1/\sqrt{K})$ fluctuations.
...
\begin{equation}
x^*_{\mathcal{K}^*}(p) = \frac{\varepsilon \cdot W(p^*_\mathcal{K})\, L_{\mathcal{K}^*}(p^*_\mathcal{K} \to p)}{W(p^*_\mathcal{K}) - W(p)} + O(\varepsilon^2), \quad p \ne p^*_\mathcal{K},
\end{equation}
...
\bottomrule
\end{tabular}
\caption{Supplementary computational figures. All scripts are run as \texttt{julia --project=.\ scripts/<script>}.}
\label{tab:supp-figures}
\end{sidewaystable}

All code and data are available at \url{https://github.com/gvdr/evo_interface_veridicality}.
Numerical summaries for these figures are recorded in \texttt{data/README.md} and the figure-specific notes in \texttt{figures/*/README.md}.
